...
import { useVisibleProjects } from "@/hooks/use-ow-workspace-visible";
...
export default function CanvasPage() {
    ...
    // OneWork 工作区隔离（P0）：只展示当前工作区的项目（存储层全量保留，不删除旧数据）。
    ...
    const agentMode = mode === "new" || mode === "recent" || mode === "choose" || mode === "open";
    ...
    useEffect(() => {
        if (!hydrated || autoOpenRef.current || (mode !== "new" && mode !== "recent" && mode !== "open")) return;
    ...
    if (hydrated && (mode === "new" || mode === "recent" || mode === "open")) return <main className="flex h-full items-center justify-center bg-background text-sm text-stone-500">{t("canvas.opening")}</main>;
